...
public class Helpers {
	 ...
	 *            the VirtualModelLibrary instance that will be used to load the VirtualModel
	 ...
	 *
	 * @param project
	 *            the project in which the virtual model will be created
	 * @param name
	...
	/**
	 * If the visibility is null, set it to an empty string. If the visibility is public, protected, or private, return the corresponding
	 * Visibility enum value. Otherwise, return the default Visibility enum value.
	 *
	 * @param visibility
	...
	public static Type getAbstractType(String type) {
		switch (type) {
			...
			default:
				return getPrimitiveType(type).getType();
		}
	}
	 ...
	 *
	 * @param type
	 *            The type of the value to be casted.
	 * @param value
	 ...
	 *
	 * @param path
	 *            the path to the folder you want to get
	 * @param project
	 *            the project to which the folder belongs
	 ...
	 *
	 * @param targetDir
	 *            The directory where the zip file will be unzipped.
	 * @param uploadedFile
	...
	public static File[] unzipFile(String targetDir, Buffer uploadedFile) throws IOException {
		...
		try {
			Files.walk(Paths.get(WORKSPACE + "uploaded_rc/__MACOSX/")).sorted(Comparator.reverseOrder()).map(Path::toFile)
					.forEach(File::delete);
		...
		try {
			Files.walk(Paths.get(WORKSPACE + "uploaded_rc/.DS_Store")).sorted(Comparator.reverseOrder()).map(Path::toFile)
					.forEach(File::delete);
		} catch (NoSuchFileException e) {
