...
public class PropertiesController extends GenericController {
	...
	/**
	 * It gets the virtual model with the given id, gets the primitive property with the given name, and returns the value of that property
	 *
	 * @param context
	 *            The routing context is the object that contains all the information about the current HTTP request and response.
	 */
	public void get(RoutingContext context) {
		...
		try {
			...
			context.response()
					.end(JsonSerializer.primitivePropertySerializer((PrimitiveRole<?>) model.getAccessibleProperty(name)).encodePrettily());
		} catch (Exception e) {
			notFound(context);
		}
	}
	...
	public void addModelSlot(RoutingContext context) {
		...
		if (validator.isValid()) {
			...
			context.response().end(
					JsonSerializer.modelSlotSerializer((AbstractFMLRTModelSlot<?, ?, ?>) modelSlot.getNewModelSlot()).encodePrettily());
		}
		else {
			badValidation(context, errors);
		}
